...
5. **Contratos públicos**
    ...
    - **Mínimo de 4 endpoints HTTP**, cada um com exemplo de payload de
      requisição, exemplo de payload de resposta e os status codes possíveis
6. **Erros, exceções e fallback**
    - Matriz de erros previstos e tratamentos
    - **Todo código de erro segue o padrão `WEBHOOK_*`** (ex: `WEBHOOK_NOT_FOUND`,
      `WEBHOOK_INVALID_URL`), consistente com o prefixo já usado no PRD/RFC/ADRs
...
9. **Integração com o sistema existente**
    - **Nomeie pelo menos 4 caminhos de arquivo reais** do código base (ex:
      `src/modules/orders/order.service.ts`, `src/shared/errors/app-error.ts`)
    - Para cada caminho, descreva concretamente como o módulo de webhooks se
      integra com ele (ex: como o `changeStatus` é estendido para publicar o
      evento na outbox dentro da mesma transação, como as classes de erro
      existentes são reutilizadas/estendidas, como o error middleware e o
      logger centralizados passam a cobrir o novo módulo)
    - Não invente caminho de arquivo; use apenas os já citados no PRD, na RFC
      ou em uma ADR
...
**[caminho/de/arquivo/real/1.ts]**
- [como o módulo se integra com este arquivo]

**[caminho/de/arquivo/real/2.ts]**
- [como o módulo se integra com este arquivo]

**[caminho/de/arquivo/real/3.ts]**
- [como o módulo se integra com este arquivo]

**[caminho/de/arquivo/real/4.ts]**
- [como o módulo se integra com este arquivo]
