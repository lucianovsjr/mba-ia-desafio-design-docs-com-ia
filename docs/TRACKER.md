...
Cobertura atual: `docs/PRD.md`, `docs/adrs/ADR-001-outbox-transacional-no-mysql.md`, `docs/adrs/ADR-002-worker-em-processo-separado-com-polling.md`, `docs/adrs/ADR-003-retry-com-backoff-e-dead-letter-queue.md` (sem linha nova: conteúdo já rastreado pelo PRD), `docs/adrs/ADR-004-hmac-sha256-com-secret-por-endpoint.md`, `docs/adrs/ADR-005-entrega-at-least-once-com-id-de-evento.md` (sem linha nova: conteúdo já rastreado pelo PRD, e a decisão de preservar o `X-Event-Id` no replay é ancorada por ID do tracker, não por citação; o agravante de duas instâncias de worker referencia `ADR-002-OPEN-01`), `docs/adrs/ADR-006-reuso-dos-padroes-existentes.md`. As linhas de RFC, FDD e das demais ADRs serão acrescentadas conforme esses documentos forem produzidos.
...
| ADR-006-DEC-02 | docs/adrs/ADR-006-reuso-dos-padroes-existentes.md | Decisão | O módulo obtém o logger pela factory `createLogger()` de `src/shared/logger/index.ts`, e não pelo singleton `logger` já exportado com `service: 'order-management-api'` fixo | CODIGO | `src/shared/logger/index.ts` |
